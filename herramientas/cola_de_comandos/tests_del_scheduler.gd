...
func test_se_le_pueden_anadir_detalles_a_los_comandos():
	yyy.registrar_nuevo_comando('sumar_x', self)
	yyy.encolar('sumar_x', {'valor_de_x': 200})
	asserts.is_true(numero == 0)
	yyy.ejecutar_siguiente_comando()
	asserts.is_true(numero == 200)
	
func sumar_3(scheduler, detalles):
	numero += 3
	scheduler.termino_la_ejecucion_del_comando()

func sumar_1(scheduler, detalles):
	numero += 1
	scheduler.termino_la_ejecucion_del_comando()

func colgarse(scheduler, detalles):
	numero += 9000
	
func sumar_x(scheduler, detalles):
